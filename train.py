...
from torch_utils.engine import evaluate, train_one_epoch, utils
...
def parse_opt():
    ...
    parser.add_argument(
        "-d", "--device", default="cuda", help="computation/training device, default is GPU if GPU present"
    )
    parser.add_argument("-e", "--epochs", default=5, type=int, help="number of epochs to train for")
    parser.add_argument(
        "-j", "--workers", default=4, type=int, help="number of workers for data processing/transforms/augmentations"
    )
    parser.add_argument("-b", "--batch", default=4, type=int, help="batch size to load the data")
    parser.add_argument("--lr", default=0.001, help="learning rate for the optimizer", type=float)
    parser.add_argument("-ims", "--imgsz", default=640, type=int, help="image size to feed to the network")
    parser.add_argument(
        "-n", "--name", default=None, type=str, help="training result dir name in outputs/training/, (default res_#)"
    )
    parser.add_argument(
        "-vt",
        "--vis-transformed",
    ...
    parser.add_argument(
        "--mosaic", default=0.0, type=float, help="probability of applying mosaic, (default, always apply)"
    )
    parser.add_argument(
        "-uta",
        "--use-train-aug",
    ...
    parser.add_argument(
        "-w", "--weights", default=None, type=str, help="path to model weights if using pretrained weights"
    )
    parser.add_argument(
        "-r",
        "--resume-training",
    ...
    parser.add_argument("--dist-url", default="env://", type=str, help="url used to set up the distributed training")
...
def main(args):
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        tensorboard_map_log(
            name="mAP", val_map_05=np.array(val_map_05), val_map=np.array(val_map), writer=writer, epoch=epoch
        )
