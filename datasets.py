...
class CustomDataset(Dataset):
    ...
    def load_image_and_labels(self, index):
        ...
        for member in root.findall("object"):
            ...
            xmin, ymin, xmax, ymax = self.check_image_and_annotation(
                xmin, ymin, xmax, ymax, image_width, image_height, orig_data=True
            )
    ...
    def load_cutmix_image_and_boxes(self, index, resize_factor=512):
        ...
        for i, index in enumerate(indices):
            _, image_resized, orig_boxes, boxes, labels, area, iscrowd, dims = self.load_image_and_labels(index=index)
            ...
            if i == 0:
                ...
                result_image = np.full(
                    (s * 2, s * 2, image_resized.shape[2]), 114 / 255, dtype=np.float32
        ...
        result_image, result_boxes = transform_mosaic(result_image, result_boxes, self.img_size)
        return result_image, torch.tensor(result_boxes), torch.tensor(np.array(final_classes)), area, iscrowd, dims
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            if self.mosaic >= mosaic_prob:
                image_resized, boxes, labels, area, iscrowd, dims = self.load_cutmix_image_and_boxes(
                    idx, resize_factor=(self.img_size, self.img_size)
                )
            else:
                image, image_resized, orig_boxes, boxes, labels, area, iscrowd, dims = self.load_image_and_labels(
                    index=idx
                )
...
def create_train_dataset(
    train_dir_images, train_dir_labels, img_size, classes, use_train_aug=False, mosaic=1.0, square_training=False
):
