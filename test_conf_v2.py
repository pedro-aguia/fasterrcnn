...
def create_frcnn():
    ...
    model.roi_heads.box_predictor = FastRCNNPredictor(in_features, len(class_mappings) + 1)
    return model
...
def predecir(model_path, imgs_path, export_path, conf_thresh, output_path, iou_thresh, save_imgs):
    ...
    for fn in img_list:
        ...
        labeled_img = next(item for item in labeled_data if item == fn)
        aux_labels = labeled_data[labeled_img]["detecciones"]
        labels, nombres = [], []
        for label in aux_labels:
            nombre = label["class_name"]
            if nombre in class_mappings.values():
                labels.append(label)
        ...
        for i in range(len(keep_results)):
            dictio = {
                ...
                "name": list(names)[keep_results.iloc[i, keep_results.columns.get_loc("class")]],
            }
            ...
            if len(iou) == 0:
                ...
                if iou[indice_aux] > iou_thresh:
                    clases_reales.append(list(names).index(categories_labeled[indice_aux]))
